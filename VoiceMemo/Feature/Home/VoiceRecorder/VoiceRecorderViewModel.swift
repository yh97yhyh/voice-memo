...
extension VoiceRecorderViewModel {
    func voiceRecorderCellTapped(_ recoredFile: URL) {
        if selectedRecordedFile != recoredFile {
            stopPlaying()
            selectedRecordedFile = recoredFile
        }
    }
...
extension VoiceRecorderViewModel {
    ...
    func getFileInfo(for url: URL) -> (Date?, TimeInterval?) {
        ...
        do {
            ...
            displayAlert(message: "선택된 음성메모 파일의 재생 시간을 불러올 수 엇습니다.")
        }
